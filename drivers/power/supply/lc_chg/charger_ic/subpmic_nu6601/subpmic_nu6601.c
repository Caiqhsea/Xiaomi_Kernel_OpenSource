...
#include "subpmic_nu6601.h"
#include "../../charger_class/lc_charger_class.h"
#ifdef CONFIG_LC_CP_CLASS
...
struct nu6601_charger *g_subpmic_nu6601 = NULL;
EXPORT_SYMBOL(g_subpmic_nu6601);
...
static int nu6601_adc_stop(struct nu6601_charger *chg)
{
	return  nu6601_reg_update_bits(chg, ADC_I2C, NU6601_REG_ADC_CTRL,
			ADC_EN_MASK, ADC_DISABLE << ADC_EN_SHIFT);
}
...
static int nu6601_enable_charger(struct nu6601_charger *chg)
{
	...
	dev_info(chg->dev, "%s: enable charger\n", __func__);
...
static int nu6601_disable_charger(struct nu6601_charger *chg)
{
	...
	dev_info(chg->dev, "%s: disable charger\n", __func__);
...
static int nu6601_hvdcp_det_enable(struct charger_dev *subpmic_dev, int qc3_en)
{
	...
	mdelay(100); // TODO: why need 100ms delay here?
	if (subpmic_dev->m_pd_active != 0 && subpmic_dev->m_pd_active != 10) {
		dev_info(chg->dev, "m_pd_active is %d\n", subpmic_dev->m_pd_active);
		return 0;
	}

	dev_info(chg->dev, "%s: enable HVDCP detect\n", __func__);
...
static int nu6601_chg_qc3_vbus_puls(struct charger_dev *subpmic_dev, bool state, int count)
{
	...
	for (i = 0; i < count; i++) {
		ret = nu6601_reg_write(chg, DPDM_I2C, NU6601_REG_QC_COT_PULSE, QC3P5_PULSE_ONE_STEP);
		if (state) {
...
static int nu6601_chg_get_qc3_result(struct charger_dev *subpmic_dev, int *res)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);

	qc35_get_qc3_type(chg->qc, res);
...
static int nu6601_chg_request_qc20(struct charger_dev *subpmic_dev, int mv)
{
	...
	dev_info(chg->dev, "vbus type %d, %dmv\n", chg->state.vbus_type, mv);
...
static int nu6601_chg_get_manufacturer_id(struct charger_dev *subpmic_dev)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);
	return chg->part_no;
}
...
__maybe_unused
static int subpmic_chg_request_dpdm(struct nu6601_charger *chg, bool en)
{
	int rc = 0;
...
static int nu6601_bc12_det_enable(struct nu6601_charger *chg, bool en)
{
	...
	if (en) {
		...
		mdelay(100);
#else
        subpmic_chg_request_dpdm(chg, true);//dpdm charger
        mdelay(100);
#endif
...
static int nu6601_set_qc_mode(struct soft_qc35 *qc, u8 mode)
{
	...
	dev_info(chg->dev, "%s: set qc mode %d\n", __func__, mode);
...
static int nu6601_set_qc_term_vbus(struct charger_dev *subpmic_dev)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);

	if (chg->qc_vbus != 5000) {
		dev_info(chg->dev, "QC charge done drop qc_vbus\n");
		return nu6601_reg_update_bits(chg, DPDM_I2C, NU6601_REG_QC_DPDM_CTRL,
...
{
	dev_info(chg->dev, "%s: val = %d hz", __func__, val);
					...
					enum flashlight_type type)
{
	...
	case FLASHLIGHT_TYPE_FLASH:
		...
		break;
	default:
		dev_err(chg->dev, "%s: unknown type:%d\n", __func__, type);
		break;
	}
	...
	*chg_status = POWER_SUPPLY_STATUS_DISCHARGING;
	...
	case CHARGE_TERMINATION:
		*chg_status = POWER_SUPPLY_STATUS_FULL;
		nu6601_set_qc_term_vbus(subpmic_dev);
		break;
	default:
		break;
	}
...
static inline enum nu6601_adc_ch to_nu6601_adc(enum sc_adc_channel channel)
{
			...
			enum sc_adc_channel channel, uint32_t *value)
{
	...
	case ADC_GET_TDIE:
		*value = adc_val;
		break;
	default:
		dev_err(chg->dev, "%s: charger adc channel %d not support\n", __func__, channel);
		ret = -1;
		break;
	}
...
static int nu6601_vac_ovp_force_off(struct charger_dev *charger, bool en)
{
	struct nu6601_charger *chg = charger_get_private(charger);
	u8 val = en ? 1 << VAC_OVP_FORCE :
		0 << VAC_OVP_FORCE;

	return nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_VAC_OVP, VAC_OVP_FORCE_MASK, val);
}

static int nu6601_enable_discharge(struct charger_dev *charger, bool en)
{
	struct nu6601_charger *chg = charger_get_private(charger);
	u8 val = en ? 1 << FORCE_ON_IPD_VBUS_SHIFT :
		0 << FORCE_ON_IPD_VBUS_SHIFT;

	return nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_IPD_CTRL, FORCE_ON_IPD_VBUS_MASK, val);
}
...
static int nu6601_chg_set_shipmode(struct charger_dev *charger, bool en)
{
	...
	dev_info(chg->dev, "%s: set ship mode,en:%d\n", __func__, en);
	subpmic_chg_set_shipmode(chg, en);

	return 0;
}

static int nu6601_chg_shipmode_count_reset(struct charger_dev *charger, bool en)
{
    struct nu6601_charger *chg = charger_get_private(charger);
    chg->ship_mode = en;
    dev_info(chg->dev, "%s: ship mode en:%d \n", __func__, chg->ship_mode);
    return 0;
}

int chargepump_nu6601_chg_set_shipmode(void)
{
    int ret = -1;

    if (!g_subpmic_nu6601) {
        pr_err("%s: g_subpmic_nu6601 is NULL\n", __func__);
        return ret;
    }

    if(g_subpmic_nu6601->ship_mode == true) {
        g_subpmic_nu6601->ship_mode = false;
        ret = subpmic_chg_set_shipmode(g_subpmic_nu6601, true);
        if (ret < 0)
            dev_info(g_subpmic_nu6601->dev, "%s: set ship mode fail\n", __func__);
        else
            dev_info(g_subpmic_nu6601->dev, "%s: set ship mode success\n", __func__);
    }

    return ret;
}
EXPORT_SYMBOL(chargepump_nu6601_chg_set_shipmode);
...
static int nu6601_enable_charging(struct charger_dev *subpmic_dev, bool en)
{
	...
	dev_info(chg->dev, "%s: en = %d\n", __func__, en);

	if (en) {
		ret  = nu6601_enable_charger(chg);
		if (ret < 0) {
			dev_err(chg->dev, "%s: set ichg fail\n", __func__);
			goto out;
		}
	} else {
...
static int nu6601_enable_boost(struct nu6601_charger *chg, bool en)
{
	...
	if (en) {
		...
		if (ret < 0) {
			nu6601_charger_key(chg, false);
			dev_err(chg->dev, "%s: nu6601_set_hiz_mode fail\n", __func__);
			return ret;
		}
...
static int nu6601_set_otg(struct charger_dev *subpmic_dev, bool en, bool fqrb_off)
{
	...
	if (fqrb_off) {
		...
		if (ret < 0) {
			dev_err(chg->dev, "%s: nu6601_force_qrb_off fail\n", __func__);
	...
	do {
		...
		if (ret < 0) {
			dev_err(chg->dev, "%s: nu6601_enable_boost fail\n", __func__);
			if (fqrb_off)
		...
		if (cnt++ > 3) {
			dev_err(chg->dev, "%s: read NU6601_REG_BOOST_OP_STAT  fail\n", __func__);
			nu6601_force_qrb_off(chg, false);
			nu6601_set_hiz_mode(subpmic_dev, false);
			return -EIO;
		}
	...
	nu6601_reg_write(chg, CHG_I2C, 0x58, 0x00);
...
int nu6601_set_otg_ovp_protect(struct charger_dev *subpmic_dev, bool en)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);

	return nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_BOOST_PORT_EN,
				BOOST_PROT_EN_MASK, !en << BOOST_PROT_EN_SHIFT);
}
...
int nu6601_set_otg_current(struct charger_dev *subpmic_dev, int mA)
{
	...
	dev_info(chg->dev, "%s: cur = %d mA", __func__, mA);
	return nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_BOOST_ILIM,
...
static int nu6601_pd_request_otg(struct charger_dev *subpmic_dev, bool en)
{
	...
	if (en)
            nu6601_set_otg_current(subpmic_dev, 1500);
...
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);
	static bool charge_current_zero_flag = false;
	int ret;

	ret = _nu6601_set_charge_current(chg, mA);
	if (!mA) {
		nu6601_disable_charger(chg);
		charge_current_zero_flag = true;
	} else if (charge_current_zero_flag) {
		ret = nu6601_enable_charger(chg);
		if (ret < 0) {
			return ret;
		}
		charge_current_zero_flag = false;
	}
...
static int nu6601_force_dpdm(struct charger_dev *subpmic_dev, bool en)
{
	struct nu6601_charger *chg = charger_get_private(subpmic_dev);
    ...
    if (en) {
        ...
        return subpmic_chg_request_dpdm(chg, false);// dpdm Ap
    }
}
...
static int nu6601_set_strobe_timeout(void *private, int ms)
{
...
int nu6601_set_torch_lvl(struct nu6601_charger *chg, u8 id, int curr)
{
	...
	if (curr > LED_OFF) {
		...
		if (ret < 0) {
			dev_err(chg->dev, "%s: set NU6601_REG_LED_CTRL ENABLE failed\n", __func__);
			return ret;
		}
	}
	...
	dev_err(chg->dev, "%s: set current id=%d curr=%d lvl=%d ret=%d\n", __func__, id, curr, lvl, ret);
	...
	if (ret < 0) {
		dev_err(chg->dev, "%s: error: id=%d curr=%d lvl=%d ret=%d\n", __func__, id, curr, lvl, ret);
	}
...
int nu6601_set_strobe_lvl(struct nu6601_charger *chg, u8 id, int curr)
{
	...
	if (down_timeout(&chg->led_semaphore, msecs_to_jiffies(800)) < 0) {
		dev_err(chg->dev, "%s: led_semaphore timeout\n", __func__);
		return -EBUSY;
	}
...
static int nu6601_adc_disable(struct nu6601_charger *chg)
{
	int ret = 0;
	...
	ret = nu6601_reg_update_bits(chg, ADC_I2C, NU6601_REG_ADC_CTRL,
		ADC_EN_MASK, ADC_DISABLE << ADC_EN_SHIFT);
	if (ret < 0) {
		dev_err(chg->dev, "%s: NU6601_REG_ADC_CTRL:%d\n", __func__, ret);
		return ret;
	}
	return ret;
}
...
static struct charger_ops nu6601_ops = {
	...
	.vac_ovp_force_off = nu6601_vac_ovp_force_off,
	.enable_discharge = nu6601_enable_discharge,
	...
	.set_otg_ovp_protect = nu6601_set_otg_ovp_protect,
	...
	.shipmode_count_reset = nu6601_chg_shipmode_count_reset,
	.set_rechg_vol = nu6601_set_rechg_volt,
	.qc_identify = nu6601_hvdcp_det_enable,
	.qc3_vbus_puls = nu6601_chg_qc3_vbus_puls,
	.get_qc3_result = nu6601_chg_get_qc3_result,
	.qc2_vbus_mode = nu6601_chg_request_qc20,
	.get_chg_manufacturer_id = nu6601_chg_get_manufacturer_id,
	.get_vindpm_state = NULL,
};
...
static int nu6601_brightness_set(void * private, enum flashlight_type type, int index, int brightness) {
    struct nu6601_charger *sy = private;
    int ret = 0;
    dev_err(sy->dev, "needcheck nu6601 brightness_set type:%d, index:%d  brightness:%d\n", type, index, brightness);
    if(brightness >= 300) {
        brightness = 300;
        dev_err(sy->dev, "force set torch mode and set nu6601 brightness_set brightness:%d\n", brightness);
    }
    ret = nu6601_set_torch_lvl(sy, 1, brightness);

    return ret;
}

static int nu6601_strobe_set(void * private, enum flashlight_type type, int index, bool state) {
    struct nu6601_charger *sy = private;
    int ret = 0;
    dev_err(sy->dev, "needcheck nu6601 strobe_set type:%d, index:%d  state:%d\n", type, index, state);

    if (state) {
		ret = nu6601_fled_set_on(sy, 1, FLASHLIGHT_TYPE_TORCH);
	} else {
		ret = nu6601_fled_set_off(sy, 1, FLASHLIGHT_TYPE_TORCH);
	}

    return ret;
}

struct subpmic_led_flash_ops led_ops = {
	.brightness_set	   = nu6601_brightness_set,
	.strobe_set        = nu6601_strobe_set,
	.flash_timeout_set = nu6601_set_strobe_timeout,
};

void nu6601_fill_leds_data(struct subpmic_flash_led* led) {
    led->private = g_subpmic_nu6601;
    led->ops = &led_ops;
}
EXPORT_SYMBOL(nu6601_fill_leds_data);
	...
	for (reg = 0x10; reg < 0x7f; reg++) {
		...
		if (buf)
			count = snprintf(buf + count, PAGE_SIZE - count,
					"[0x%x] -> 0x%x\n", reg, val);
		dev_info(chg->dev, "nu6601 reg 0x33: [0x%x] -> 0x%x\n", reg, val);
	}
	nu6601_charger_key(chg, false);
...
static void nu6601_sdp_chg_workfunc(struct work_struct *work)
{
	struct nu6601_charger *chg = container_of(work, struct nu6601_charger, sdp_chg_work.work);
	u8 val = 0;

	if ((chg->state.vbus_type != VBUS_TYPE_SDP) || (!chg->state.online))
		return;

	nu6601_reg_read(chg, CHG_I2C, NU6601_REG_CHG_DIS_NOK_FLAG, &val);
	if ((chg->state.vbus_type == VBUS_TYPE_SDP) && (val == FAST_CHG_TIMER_EXP)
			&& !(_nu6601_get_chg_type(chg) == CHARGE_TERMINATION)) {
		dev_err(chg->dev, "%s: SDP charger timeout\n", __func__);
		nu6601_set_hiz_mode(chg->subpmic_dev, true);
		nu6601_set_hiz_mode(chg->subpmic_dev, false);
	}

	schedule_delayed_work(&chg->sdp_chg_work, msecs_to_jiffies(2000));
}
...
static void nu6601_first_cid_det_workfunc(struct work_struct *work)
{
	...
	if (chg->chg_cfg.cid_en) {
		...
		if ((bc12_int_stat & 0x10) == 0x10) {
			val = 1;
			dev_info(chg->dev, "CID activated and USB plugged in");
		} else {
			val = 0;
			dev_info(chg->dev, "CID activated and USB plugged out");
		}
...
static void nu6601_update_state(struct nu6601_charger *chg)
{
	...
	if (ret < 0) {
		dev_err(chg->dev, "%s: read NU6601_REG_VBUS_DET_STATUS error\n", __func__);
	} else {
...
static irqreturn_t nu6601_vbus_0v_irq_handler(int irq, void *data)
{
	...
	if (chg->state.online == 0) {
		...
		cancel_delayed_work_sync(&chg->sdp_chg_work);
		fsw = nu6601_get_buck_fsw(chg);
		if (fsw == BUCK_FSW_SEL_500K) {
			nu6601_set_hiz_mode(chg->subpmic_dev, true);
			nu6601_set_buck_fsw(chg, BUCK_FSW_SEL_1000K);
	...
	dev_info(chg->dev, "%s: irq handle success\n", __func__);
...
#else
	if ((chg->state.vbus_type == VBUS_TYPE_SDP) || (chg->state.vbus_type == VBUS_TYPE_CDP)) {
		subpmic_chg_request_dpdm(chg, false);// dpdm Ap
	}
#endif

	if ((chg->state.vbus_type == VBUS_TYPE_SDP) && (chg->state.online))
		schedule_delayed_work(&chg->sdp_chg_work, msecs_to_jiffies(5000));
...
static int nu6601_set_led_flash(struct nu6601_charger *chg, int index)
{
	...
	dev_info(chg->dev, "%s: index:%d\n", __func__, index);

	if (down_timeout(&chg->led_semaphore, msecs_to_jiffies(500)) < 0) {
		dev_err(chg->dev, "led_semaphore timeout\n");
		return -EBUSY;
	}
...
static void nu6601_led_flash_done_workfunc(struct work_struct *work)
{
	...
	if (!wait_for_completion_timeout(&chg->led_complete,
		...
		dev_err(chg->dev, "%s: led flash timeout failed", __func__);
	}
...
static int nu6601_charger_init_setting(struct nu6601_charger *chg)
{
	...
	ret |= nu6601_set_otg_current(chg->subpmic_dev, 1500);
	...
	ret |= nu6601_reg_update_bits(chg, CHG_I2C, NU6601_REG_BOOST_PORT_CFG,
			VBAT_LOW_MASK, VBAT_LOW_2P8V << VBAT_LOW_SHIFT);
	...
	ret |= subpmic_chg_request_dpdm(chg, true);// usb dpdm hiz default for charger
...
static inline int nu6601_charger_parse_dt(struct device *dev, struct nu6601_charger *chg)
{
	...
	if (ret < 0) {
		dev_err(chg->dev, "%s: use default torch cur\n", __func__);
	...
	if (ret) {
		...
		dev_err(chg->dev, "%s: use default strobe cur\n", __func__);
	} else {
	...
	if (ret) {
		dev_err(chg->dev, "%s: use default strobe timeout\n", __func__);
		...
		const char *buf, size_t count)
{
	...
	if (ret < 0) {
		dev_err(chg->dev, "get parameters fail\n");
		return -EINVAL;
	}
	switch (val) {
	case 1: /* enable otg */
		...
		int index, int brightness)
{
	...
	dev_info(chg->dev, "%s: flash type:%d, index:%d  brightness:%d\n",
			__func__, type, index, brightness);
...
int strobe_set(void *private, enum flashlight_type type, int index, bool state)
{
	...
	dev_err(chg->dev, "%s: flash type:%d, index:%d, state:%d\n",
			__func__, type, index, state);
...
static int nu6601_charger_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: driver version(roger: %s)\n", __func__, NU6601_CHARGER_DRV_VERSION);
	...
	if (chg->part_no == NU6601_DEV_ID) {
		dev_err(&pdev->dev, "%s: charger device nu6601 detected, rev id 0x%x\n",
				__func__, chg->revision);
	} else {
		dev_err(&pdev->dev, "%s: no charger device found:%d\n", __func__, ret);
		return -ENODEV;
	}
	...
	chg->subpmic_dev = charger_register("primary_chg", chg->dev, &nu6601_ops, chg);
	...
	/* Register led device */
	chg->led_dev = subpmic_led_register("subpmic_led", chg->dev, &nu6601_led_ops, chg);
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: parse dts failed\n", __func__);
		goto err_dtb;
	}

	INIT_DELAYED_WORK(&chg->buck_fsw_work, nu6601_buck_fsw_workfunc);
	INIT_DELAYED_WORK(&chg->sdp_chg_work, nu6601_sdp_chg_workfunc);

	/* SW workaround */
	ret = nu6601_charger_sw_workaround(chg);
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: software workaround failed\n", __func__);
		goto err_wa;
	}
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: sw init failed\n", __func__);
		goto err_init;
	}
