// SPDX-License-Identifier: GPL-2.0
/**
 * Copyright (c) 2022 - 2023 Southchip Semiconductor Technology(Shanghai) Co., Ltd.
 */
#include <linux/i2c.h>
#include <linux/init.h>
#include <linux/interrupt.h>
#include <linux/irq.h>
#include <linux/irqdomain.h>
#include <linux/module.h>
#include <linux/mutex.h>
#include <linux/of_platform.h>
#include <linux/regmap.h>
#include <linux/atomic.h>
#include <linux/of.h>
#include <linux/gpio.h>

//#include "subpmic_irq.h"
#include "subpmic.h"
#include "../../../../../usb/typec/tcpc/inc/tcpci_typec.h"
#include "../../charger_policy/lc_charger_manager.h"
#include "../../../../../usb/typec/tcpc/inc/tcpm.h"

#define SUBPMIC_CID_VERSION             "0.0.1"
#define TYPEC_PORT_NAME                 "type_c_port0"

struct subpmic_cid_device {
    struct i2c_client *client;
    struct device *dev;
    struct regmap *rmap;

    struct work_struct cid_work;
    atomic_t cid_pending;
    bool pd_ready;
    int part_no;
    struct tcpc_device *tcpc;
    struct charger_manager *cm;
    struct delayed_work cid_dwork;
};

#define SUBPMIC_CID_FLAG            BIT(7)

#define SC6601_DEVICE_ID        0X66
#define SC6601_1P1_DEVICE_ID    0x61
#define SC6601A_DEVICE_ID       0x62

static void cid_detect_workfunc(struct work_struct *work)
{
    static struct power_supply *psy_usb = NULL;
    struct subpmic_cid_device *sc = container_of(work,
                struct subpmic_cid_device, cid_work);
    int ret = 0;
    u8 state = 0;
    int typec_role = 0;
    int cid_status = 0;

    if (!sc->cm) {
        if (!psy_usb) {
            psy_usb = power_supply_get_by_name("usb");
            if (!psy_usb)
                dev_err(sc->dev, "%s get usb psy failed\n", __func__);
        }
        if (psy_usb) {
            sc->cm = power_supply_get_drvdata(psy_usb);
            if (!sc->cm)
                dev_err(sc->dev, "%s get charger manager failed\n", __func__);
        }
    }

    while(!sc->pd_ready) {
        dev_err(sc->dev, "%s \n", __func__);
        sc->tcpc = tcpc_dev_get_by_name(TYPEC_PORT_NAME);
        if (sc->tcpc) {
            typec_role = tcpm_inquire_typec_role(sc->tcpc);

            if (typec_role > TYPEC_ROLE_UNKNOWN &&

                typec_role < TYPEC_ROLE_NR && sc->tcpc->pd_inited_flag) {
                break;
            }
        }

        msleep(1000);
    }

    do {
        ret = regmap_bulk_read(sc->rmap, SUBPMIC_REG_HK_GEN_STATE, &state, 1);
        if (ret) {
            dev_err(sc->dev, "failed to read irq state\n");
            return;
        }

        if (state & SUBPMIC_CID_FLAG) {
            cid_status = 1;
            dev_info(sc->dev, "cid detect plug-in\n");
#ifndef CONFIG_FACTORY_BUILD
            if (!sc->cm || !sc->cm->ui_cc_toggle) {
                dev_info(sc->dev, "cid cancel try snk mode\n");
#endif
                if (sc->pd_ready) {
                    tcpm_typec_change_role_postpone(sc->tcpc, TYPEC_ROLE_TRY_SNK, true);
                } else {
                    tcpm_typec_change_role_postpone(sc->tcpc, TYPEC_ROLE_TRY_SNK, false);
                }
#ifndef CONFIG_FACTORY_BUILD
            }
#endif
        } else {
            cid_status = 0;
            dev_info(sc->dev, "cid detect plug-out\n");
#ifndef CONFIG_FACTORY_BUILD
            if (!sc->cm || !sc->cm->ui_cc_toggle) {
                dev_info(sc->dev, "cid cancel set snk mode\n");
#endif
                tcpm_typec_change_role_postpone(sc->tcpc, TYPEC_ROLE_SNK, true);
#ifndef CONFIG_FACTORY_BUILD
            }
#endif
        }
        atomic_dec_if_positive(&sc->cid_pending);
        sc->pd_ready = true;
    } while(atomic_read(&sc->cid_pending));

#ifndef CONFIG_FACTORY_BUILD
    if (sc->cm) {
        // ui cc toggle switch off
        if (!sc->cm->ui_cc_toggle) {
            if (cid_status) {
                dev_info(sc->dev, "sc6601 schedule cid work\n");
                pm_stay_awake(sc->dev);
                schedule_delayed_work(&sc->cid_dwork, msecs_to_jiffies(5000));
            } else {
                dev_info(sc->dev, "sc6601 cancle cid work\n");
                cancel_delayed_work(&sc->cid_dwork);
                pm_relax(sc->dev);
            }
        }
        sc->cm->cid_plugin = cid_status;
    }
#endif
}

#ifndef CONFIG_FACTORY_BUILD
static void cid_wait5s_work(struct work_struct *cid_work)
{
    struct subpmic_cid_device *sc = container_of(cid_work, struct subpmic_cid_device, cid_dwork.work);

    // otg switch off && cid pin float && no typec device
    if (sc->cm && !sc->cm->ui_cc_toggle && sc->cm->cid_plugin && !sc->cm->typec_attach && !sc->cm->audio_cctog) {
        if (!sc->cm->swcid_bright) {
            dev_info(sc->dev, "sc6601 switch to snk mode\n");
            tcpm_typec_change_role_postpone(sc->tcpc, TYPEC_ROLE_SNK, true);
        } else {
            dev_info(sc->dev, "sc6601 switch to try snk mode\n");
            tcpm_typec_change_role_postpone(sc->tcpc, TYPEC_ROLE_TRY_SNK, true);
        }
    }
    pm_relax(sc->dev);
}
#endif

static irqreturn_t subpmic_cid_alert_handler(int irq, void *data)
{
    struct subpmic_cid_device *sc = data;
    atomic_inc(&sc->cid_pending);
    schedule_work(&sc->cid_work);
    return IRQ_HANDLED;
}

int sc6601_detect_device(struct subpmic_cid_device *sc)
{
    int ret;
    uint8_t val = 0;

    ret = regmap_bulk_read(sc->rmap, SUBPMIC_REG_DEVICE_ID, &val, 1);
    if (ret == 0) {
        sc->part_no = val;
    }

    return ret;
}

static int subpmic_cid_probe(struct platform_device *pdev)
{
    struct device *dev = &pdev->dev;
    struct subpmic_cid_device *sc;
    int ret = 0;

    dev_info(dev, "%s (%s)\n", __func__, SUBPMIC_CID_VERSION);
#ifdef CONFIG_FACTORY_BUILD
    dev_info(dev, "fac no cid %s (%s)\n", __func__, SUBPMIC_CID_VERSION);
    return 0;
#endif
    sc = devm_kzalloc(dev, sizeof(*sc), GFP_KERNEL);
    if (!sc)
        return -ENOMEM;
    sc->rmap = dev_get_regmap(dev->parent, NULL);
    if (!sc->rmap) {
        dev_err(dev, "failed to get regmap\n");
        return -ENODEV;
    }

    sc->dev = dev;
    platform_set_drvdata(pdev, sc);
    device_init_wakeup(dev, true);
#ifndef CONFIG_FACTORY_BUILD
    INIT_DELAYED_WORK(&sc->cid_dwork, cid_wait5s_work);
#endif

    ret = sc6601_detect_device(sc);
    if (ret == 0 && (sc->part_no == SC6601_DEVICE_ID || sc->part_no == SC6601_1P1_DEVICE_ID ||
            sc->part_no == SC6601A_DEVICE_ID)) {
        dev_err(sc->dev, "%s: charger device sc6601 detected part_no:%d\n", __func__, sc->part_no);
    } else {
        dev_err(sc->dev, "%s: no charger device found:%d\n", __func__, ret);
        return -ENODEV;
    }

    INIT_WORK(&sc->cid_work, cid_detect_workfunc);
    atomic_set(&sc->cid_pending, 0);

    ret = platform_get_irq_byname(to_platform_device(sc->dev), "CID");
    if (ret < 0) {
        dev_err(sc->dev, "failed to get irq CID\n");
        return ret;
    }

    ret = devm_request_threaded_irq(sc->dev, ret, NULL,
                subpmic_cid_alert_handler, IRQF_ONESHOT,
                dev_name(sc->dev), sc);
    if (ret < 0) {
        dev_err(sc->dev, "failed to request irq CID\n");
        return ret;
    }

    schedule_work(&sc->cid_work);
    sc->pd_ready = false;

    dev_info(dev, "%s probe success\n", __func__);

    return 0;
}

static int subpmic_cid_remove(struct platform_device *pdev)
{
    return 0;
}

static void subpmic_cid_shutdown(struct platform_device *pdev)
{

}

static const struct of_device_id subpmic_cid_of_match[] = {
    {.compatible = "southchip,subpmic_cid",},
    {},
};

static struct platform_driver subpmic_cid_driver = {
    .driver = {
        .name = "subpmic_cid",
        .of_match_table = of_match_ptr(subpmic_cid_of_match),
    },
    .probe = subpmic_cid_probe,
    .remove = subpmic_cid_remove,
    .shutdown = subpmic_cid_shutdown,
};

module_platform_driver(subpmic_cid_driver);

MODULE_AUTHOR("Boqiang Liu <air-liu@southchip.com>");
MODULE_DESCRIPTION("sc6601A CID driver");
MODULE_LICENSE("GPL v2");
