...
struct subpmic_chg_device *g_subpmic_sc6601 = NULL;
EXPORT_SYMBOL(g_subpmic_sc6601);
...
static int subpmic_chg_field_read(struct subpmic_chg_device *sc, enum subpmic_chg_fields field_id)
{
    ...
    if (ret < 0) {
        dev_err(sc->dev, "i2c field read failed fields=%d ret=%d\n", field_id, ret);
        return ret;
    }
...
static int subpmic_chg_field_write(struct subpmic_chg_device *sc, enum subpmic_chg_fields field_id, u8 val)
{
    ...
    if (ret < 0) {
        dev_err(sc->dev, "i2c field write failed fields=%d ret=%d\n", field_id, ret);
    }
    return ret;
}
                                ...
                                u8 *val,size_t count)
{
    ...
    if (ret < 0) {
        dev_err(sc->dev, "i2c bulk read failed reg=%d ret=%d\n", reg, ret);
    }
    return ret;
}
                                ...
                                u8 *val, size_t count)
{
    ...
    if (ret < 0) {
        dev_err(sc->dev, "i2c bulk write failed reg=%d ret=%d\n", reg, ret);
    }
    return ret;
}
...
__maybe_unused
static int subpmic_chg_get_qc3_result(struct subpmic_chg_device *sc, int *res)
{
    *res = sc->qc_result;
...
static int subpmic_chg_set_input_curr_lmt(struct subpmic_chg_device *sc, int ma)
{
    ...
    dev_info(sc->dev, "set iindpm is  %d\n", ma);
    if (ma <= 100) {
        /* In Preformance Mode (Bcuk off, Regn ON) */
        return subpmic_chg_field_write(sc, F_PERFORMANCE_EN, true);
    }

    ret = subpmic_chg_field_write(sc, F_PERFORMANCE_EN, sc->input_suspend);
    if (ret < 0)
        return ret;
...
static int subpmic_chg_set_input_volt_lmt(struct subpmic_chg_device *sc, int mv)
{
    ...
    ret = subpmic_chg_field_write(sc, F_VINDPM, SC6601_REG33_VDPM_4_6V);
#else
    ret = subpmic_chg_field_write(sc, F_VINDPM, i);
    dev_info(sc->dev, "set vindpm is  %d, reg:0x%x\n", mv, i);
#endif
    if (ret < 0)
        return ret;
...
__maybe_unused
static int subpmic_chg_set_vindpm_track(struct subpmic_chg_device *sc, int mv)
{
    return subpmic_chg_field_write(sc, F_VINDPM_VBAT, mv);
}
...
{
    dev_info(sc->dev, "set chg en:%d request_otg:%d sc->vindpm_set_flag:%d", en, sc->request_otg, sc->vindpm_set_flag);

    if (!sc->vindpm_set_flag)
        return 0;

    if (sc->request_otg && en)
        return 0;

    if (sc->state.vindpm == vindpm[ARRAY_SIZE(vindpm) - 1]) {
        dev_info(sc->dev, "vindpm is high, dis chg\n");
        return subpmic_chg_field_write(sc, F_CHG_EN, false);
    }
...
static int subpmic_chg_get_chg(struct subpmic_chg_device *sc)
{
    ...
    dev_info(sc->dev, "get chg en:%d", ret);
...
static int subpmic_chg_set_otg(struct subpmic_chg_device *sc, bool en)
{
    ...
    dev_info(sc->dev, "set otg en:%d", en);
    do {
        boost_state = 0;
    ...
    sc->except_otg_flag = en;
    return 0;
}

static int subpmic_chg_get_vindpm_status(struct subpmic_chg_device *sc)
{
    int ret;
    int val;
    u8 vindpm_status;
    ret = subpmic_chg_read_byte(sc, SUBPMIC_REG_CHG_INT_STAT + 2, &vindpm_status);
    if (ret < 0) {
        dev_err(sc->dev, "i2c bulk read vindpm_status failed ret=%d\n", ret);
        return ret;
    }
    val = !!(vindpm_status & SUBPMIC_VINDPM_STATE);
    return val;
}
...
__maybe_unused
static int subpmic_chg_set_otg_ovp_protect(struct subpmic_chg_device *sc, bool en)
{
    return subpmic_chg_field_write(sc, F_VPMID_OVP_OTG_DIS, en);
}
...
__maybe_unused
static int subpmic_pd_enter_shutdown(struct subpmic_chg_device *sc)
{
    int ret = 0;

    /* must reset pd phy */
    uint8_t shutdown_data[] = {0xA0, 0x01};

    struct i2c_msg msg = {
        .addr = SUBPMIC_PD_I2C_ADDR,
        .flags = 0,
    };
    msg.len = sizeof(shutdown_data);
    msg.buf = shutdown_data;
    ret = i2c_transfer(sc->adp, &msg, 1);
    if (ret < 0) {
        dev_err(sc->dev, "I2C write failed: %d\n", ret);
        return ret;
    }
    return ret;
}
...
static int subpmic_chg_set_shipmode(struct subpmic_chg_device *sc, bool en)
{
    ...
    if (en) {
        /* close adc */
        ret = subpmic_chg_field_write(sc, F_ADC_EN, false);
        /* close pd */
        ret |=subpmic_pd_enter_shutdown(sc);
...
static int subpmic_chg_request_dpdm(struct subpmic_chg_device *sc, bool en)
{
	...
	if (en) {
		if (sc->dpdm_reg && !sc->dpdm_enabled) {
			...
			rc = regulator_enable(sc->dpdm_reg);
			if (rc < 0)
		...
		if (sc->dpdm_reg && sc->dpdm_enabled) {
			...
			rc = regulator_disable(sc->dpdm_reg);
			if (rc < 0)
...
static int subpmic_chg_force_dpdm(struct subpmic_chg_device *sc)
{
    if (sc->use_soft_bc12)
        ...
        Charger_Detect_Init();
#else
        subpmic_chg_request_dpdm(sc, true);//dpdm charger
#endif
...
#else
            if ((result == VBUS_TYPE_SDP) || (result == VBUS_TYPE_CDP)) {
                subpmic_chg_request_dpdm(sc, false);// dpdm Ap
            }
#endif
...
static void vindpm_reset_workfunc(struct work_struct *work)
{
    struct subpmic_chg_device *sc = container_of(work,
                struct subpmic_chg_device, vindpm_reset_work.work);

    sc->vindpm_set_flag = true;
    if (sc->sw_vindpm)
	    subpmic_chg_set_input_volt_lmt(sc, sc->sw_vindpm);
...
static irqreturn_t subpmic_chg_hk_alert_handler(int irq, void *data)
{
    ...
    if (!last_online && sc->state.online) {
        dev_info(sc->dev, "plug in\n");
        if (sc->flag_variable) {
            subpmic_chg_set_input_volt_lmt(sc, 8100);
            subpmic_chg_set_chg(sc, false);
            sc->vindpm_set_flag = false;
            schedule_delayed_work(&sc->vindpm_reset_work, msecs_to_jiffies(1500));
        }
    ...
    if (last_online && !sc->state.online) {
        ...
        cancel_delayed_work(&sc->vindpm_reset_work);
        sc->vindpm_set_flag = true;
        // relese qc
        subpmic_chg_field_write(sc, F_QC_EN, false);
        sc->state.vbus_type = NONE;
#ifdef CONFIG_FACTORY_BUILD
    } else if(!last_online && sc->state.online) {
        // vbus断开后重新设置vindpm_track为300mv
        ret |= subpmic_chg_set_vindpm_track(sc, SC6601_REG33_VDPM_BAT_TRACK_300MV);
        if (ret < 0) {
            dev_err(sc->dev, "set sc6601 vindpm vol failed\n");
        } else {
            dev_info(sc->dev, "Set vindpm_track to 300mv successfully\n");
        }
#endif
    }
...
__maybe_unused
static int subpmic_chg_set_led_flash_timer(void *private, int ms)
{
    int i = 0;
    struct subpmic_chg_device *sc = private;
...
static int subpmic_set_led_flash_curr(struct subpmic_led_dev *led_dev, enum SUBPMIC_LED_ID id, int ma)
{
    ...
    return subpmic_chg_set_led_flash_curr(sc, 0, ma);
}
__maybe_unused
...
{
    struct subpmic_chg_device *sc = subpmic_led_get_private(led_dev);
    return subpmic_chg_set_led_torch_enable(sc, 0, en);
}
...
{
    struct subpmic_chg_device *sc = container_of(work,
                    struct subpmic_chg_device, otg_retry_work.work);
    uint8_t boost_reg;
    boost_reg = subpmic_chg_field_read(sc, F_BOOST_EN);
    if (boost_reg != sc->except_otg_flag)
    {
        dev_info(sc->dev, "reset otg %d\n", sc->except_otg_flag);
        subpmic_chg_field_write(sc, F_BOOST_EN, sc->except_otg_flag);
    }
}
...
static int subpmic_chg_hw_init(struct subpmic_chg_device *sc)
{
    ...
    ret |= subpmic_chg_set_vac_ovp(sc, 11000);

    ret |= subpmic_chg_set_vbus_ovp(sc, 14000);

    ret |= subpmic_chg_set_ico_enable(sc, false);

       ret |= subpmic_chg_set_otg_volt(sc, 5200);

    ret |= subpmic_chg_set_otg_curr(sc, 1500);
    ret |= subpmic_chg_set_input_curr_lmt(sc, 1000);
    ...
    ret |= subpmic_chg_write_byte(sc, 0x75, 0x9);

#ifdef CONFIG_FACTORY_BUILD
    /*enable vindpm_track=300mv*/
    ret |= subpmic_chg_set_vindpm_track(sc, SC6601_REG33_VDPM_BAT_TRACK_300MV);
    if (ret < 0) {
		dev_err(sc->dev, "set sc6601 vindpm vol failed\n");
		return ret;
...
static int sc_chg_set_otg(struct charger_dev *charger, bool en)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    if (en) {
        subpmic_chg_set_otg_volt(sc, 5400);
        subpmic_chg_set_otg_curr(sc, 2100);
    }
...
static int sc_chg_set_otg_ovp_protect(struct charger_dev *charger, bool en)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    return subpmic_chg_set_otg_ovp_protect(sc, en);
}

static int sc_chg_get_vindpm_status(struct charger_dev *charger)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    return subpmic_chg_get_vindpm_status(sc);
}
...
static int sc_chg_request_dpdm(struct charger_dev *charger, bool en)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    if (en) {
        return subpmic_chg_force_dpdm(sc);
    } else {
        return subpmic_chg_request_dpdm(sc, false);// dpdm Ap
    }
}
...
static int sc_chg_set_shipmode(struct charger_dev *charger, bool en)
{
    ...
    dev_info(sc->dev, "%s: set ship mode start  \n", __func__);
    ret = subpmic_chg_set_shipmode(sc, en);
    if (ret < 0)
        dev_err(sc->dev, "%s: set ship mode fail\n", __func__);
    else
        dev_err(sc->dev, "%s: set ship mode success\n", __func__);

    return ret;
}

static int sc_chg_shipmode_count_reset(struct charger_dev *charger, bool en)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    sc->ship_mode = en;
...
int chargepump_sc6601_chg_set_shipmode(void)
{
    int ret = -1;

    if (!g_subpmic_sc6601) {
        pr_err("%s: g_subpmic_sc6601 is NULL\n", __func__);
        return ret;
    }

    if(g_subpmic_sc6601->ship_mode == true) {
        ret = subpmic_chg_set_shipmode(g_subpmic_sc6601, true);
        if (ret < 0)
            dev_info(g_subpmic_sc6601->dev, "%s: set ship mode fail\n", __func__);
        else
            dev_info(g_subpmic_sc6601->dev, "%s: set ship mode success\n", __func__);
    }

    return ret;
}
EXPORT_SYMBOL(chargepump_sc6601_chg_set_shipmode);
...
static int sc_chg_get_qc3_result(struct charger_dev *charger, int *res)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    return subpmic_chg_get_qc3_result(sc, res);
}
...
static int sc_chg_get_manufacturer_id(struct charger_dev *charger)
{
    struct subpmic_chg_device *sc = charger_get_private(charger);
    return sc->part_no;
}
...
static struct charger_ops charger_ops = {
    ...
    .set_otg_ovp_protect = sc_chg_set_otg_ovp_protect,
    .get_vindpm_status = sc_chg_get_vindpm_status,
    ...
    .force_dpdm = sc_chg_request_dpdm,
    .reset = sc_chg_reset,
    .set_wd_timeout = sc_chg_set_wd_timeout,
    .kick_wd = sc_chg_kick_wd,
    .set_shipmode = sc_chg_set_shipmode,
    .shipmode_count_reset = sc_chg_shipmode_count_reset,
    .set_rechg_vol = sc_chg_set_rechg_vol,
    .qc_identify = sc_chg_qc_identify,
    .qc3_vbus_puls = sc_chg_qc3_vbus_puls,
    .get_qc3_result = sc_chg_get_qc3_result,
    .qc2_vbus_mode = sc_chg_request_qc20,
    .get_chg_manufacturer_id = sc_chg_get_manufacturer_id,
};

static int brightness_set(void * private, enum flashlight_type type, int index, int brightness) {
    struct subpmic_chg_device *sc = private;
    int ret = 0;
    dev_info(sc->dev, "needcheck sc6601 brightness_set type:%d, index:%d  brightness:%d\n", type, index, brightness);

    if(brightness >= 300) {
        brightness = 300;
        dev_err(sc->dev, "force set torch mode and set sc6601 brightness_set brightness:%d\n", brightness);
    }
    ret = subpmic_chg_set_led1_torch_curr(sc, brightness);

    return ret;
}

static int strobe_set(void * private, enum flashlight_type type, int index, bool state) {
    struct subpmic_chg_device *sc = private;
    int ret = 0;
    dev_info(sc->dev, "needcheck sc6601 strobe_set type:%d, index:%d  state:%d\n", type, index, state);

    ret = subpmic_chg_led1_torch_enable(sc, state);
...
struct subpmic_led_flash_ops led_ops = {
	...
	.strobe_set        = strobe_set,
	.flash_timeout_set = subpmic_chg_set_led_flash_timer,
};

void sc6601_fill_leds_data(struct subpmic_flash_led* led) {
    led->private = g_subpmic_sc6601;
    led->ops = &led_ops;
}
EXPORT_SYMBOL(sc6601_fill_leds_data);
...
int sc6601_detect_device(struct subpmic_chg_device *sc)
{
      int ret;
      uint8_t val = 0;

  	ret = regmap_bulk_read(sc->rmap, SUBPMIC_REG_DEVICE_ID, &val, 1);
  	if (ret == 0) {
  		sc->part_no = val;
  	}
...
static void flag_timer_callback(struct timer_list *t)
{
    struct subpmic_chg_device *sc = from_timer(sc, t, flag_timer);
    sc->flag_variable = 1;
    printk(KERN_INFO "Variable set to 1 after 10 seconds\n");
}
static int subpmic_chg_probe(struct platform_device *pdev)
{
    ...
    sc->sw_vindpm = 0;
    sc->vindpm_set_flag = true;
    ret = sc6601_detect_device(sc);
    if (ret == 0 && (sc->part_no == SC6601_DEVICE_ID || sc->part_no == SC6601_1P1_DEVICE_ID ||
  			sc->part_no == SC6601A_DEVICE_ID)) {
        dev_err(sc->dev, "%s: charger device sc6601 detected part_no:%d\n", __func__, sc->part_no);
    } else {
        dev_err(sc->dev, "%s: no charger device found:%d\n", __func__, ret);
  	return -ENODEV;
    }
    ...
    timer_setup(&sc->flag_timer, flag_timer_callback, 0);
    mod_timer(&sc->flag_timer, jiffies + msecs_to_jiffies(15000));
    ...
    ret = subpmic_chg_field_write(sc, F_BUCK_FREQ, 1);//1Mhz
    if (ret < 0)
        dev_err(dev, "F_BUCK_FREQ 1Mhz failed\n");
    ...
    INIT_DELAYED_WORK(&sc->vindpm_reset_work, vindpm_reset_workfunc);
...
//#endif /* CONFIG_ENABLE_SYSFS_DEBUG */
    ...
    timer_setup(&sc->bc12_timeout, bc12_timeout_func, 0);
...
static void subpmic_chg_shutdown(struct platform_device *pdev)
{
    ...
    //if (g_subpmic_sc6601->ship_mode == false)
    ...
    ret = subpmic_chg_adc_enable(sc, false);
    ret = subpmic_chg_field_read(sc, F_ADC_EN);

    dev_info(sc->dev, "%s 0fh:0x%x\n", __func__,ret);
}
