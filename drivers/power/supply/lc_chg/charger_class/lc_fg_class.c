...
int fuel_gauge_set_shipmode(struct fuel_gauge_dev *fuel_gauge, bool en)
{
	if (!fuel_gauge || !fuel_gauge->ops)
		return -EINVAL;
	if (fuel_gauge->ops->set_shipmode == NULL)
		return -EOPNOTSUPP;
	return fuel_gauge->ops->set_shipmode(fuel_gauge, en);
}
EXPORT_SYMBOL(fuel_gauge_set_shipmode);
