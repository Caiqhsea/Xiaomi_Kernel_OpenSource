...
int adapter_get_usbpd_verifed(struct adapter_device *adapter_dev, bool *verifed)
{
	if (adapter_dev != NULL && adapter_dev->ops != NULL &&
	    adapter_dev->ops->get_usbpd_verifed)
		return adapter_dev->ops->get_usbpd_verifed(adapter_dev, verifed);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(adapter_get_usbpd_verifed);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (adapter_dev != NULL && adapter_dev->ops != NULL &&
		adapter_dev->ops->get_current_status) {
		adapter_dev->ops->get_current_status(adapter_dev);
	}
...
struct adapter_device *adapter_device_register(const char *name,
		...
		const struct xm_adapter_ops *ops,
