...
*/
...
#else
			}else if (manager->usb_online && manager->fake_batt_status_hot == POWER_SUPPLY_STATUS_FULL) {
#endif
			...
			if (vindpm_status && ibus < 100 && vbus < manager->vindpm_vot) {
				...
				manager->vindpm_dischg_flag = true;
				status = POWER_SUPPLY_STATUS_DISCHARGING;
			} else if (vindpm_status && ibus < 100 && manager->vindpm_dischg_flag) {
				status = POWER_SUPPLY_STATUS_DISCHARGING;
				lc_err("wait vindpm_status remove or ibus >= 100\n");
			} else {
				manager->vindpm_dischg_flag = false;
			}
			...
			if(sm->fake_cycle_count >= 0) {
				...
				if (CHIP_ONLINE == ll_check_secret_chip_online()) {
					...
					if (chip_cycle >= 0) {
						...
						lc_info("use chip cycle:%d\n", val->intval);
						break;
					} else {
						lc_info("use platform  cycle:%d\n", val->intval);
					}
				}
		...
		case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
			...
			if (ret < 0) {
				lc_err("failed to get charge_full_design prop\n");
				break;
			}
			val->intval = pval.intval;
			break;
...
int audio_get_prop_typec_mode(struct power_supply *psy)
{
	struct charger_manager *manager = power_supply_get_drvdata(psy);
	...
	return manager->tcpc->typec_mode;
}
...
#if IS_ENABLED(CONFIG_REVERSE_18W) 
...
static ssize_t first_usage_date_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
