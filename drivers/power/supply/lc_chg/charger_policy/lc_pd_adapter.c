...
#ifdef TAG
#undef TAG
#define  TAG "[LC_CHG][adapter]"
#endif

/* PD */
#include "../../../../usb/typec/tcpc/inc/tcpm.h"
#include "../charger_class/lc_adapter_class.h"
											...
											uint32_t mv, uint32_t ma)
{
	...
	if(info->pd_cap.type[nr] == TCPM_POWER_CAP_VAL_TYPE_AUGMENT
		...
		ret = tcpm_set_apdo_charging_policy(info->tcpc, (DPM_CHARGING_POLICY_PPS | DPM_CHARGING_POLICY_IGNORE_MISMATCH_CURR), mv, ma, NULL);
...
static int pd_adapter_probe(struct platform_device *pdev)
{
	...
	if (info->tcpc == NULL) {
		...
		if (probe_cnt >= PROBE_CNT_MAX) {
			lc_err("%s: failed to get tcpc device\n", __func__);
			ret = -EINVAL;
		}
	...
	if (ret < 0) {
		lc_err("register tcpc notifer fail\n");
		ret = -EINVAL;
		goto err_get_tcpc_dev;
	}
	lc_info("successfully!\n");
	return 0;
...
MODULE_DESCRIPTION("LC PD Adapter Class");
