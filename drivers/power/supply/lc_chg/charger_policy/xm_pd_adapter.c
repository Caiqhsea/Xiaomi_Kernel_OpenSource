...
static int pd_get_svid(struct adapter_device *dev)
{
	...
	if (ret == TCPM_SUCCESS) {
		...
		if (ret == TCPM_SUCCESS) {
			...
			adapter_err("[%s] get adapter message failed ret=%d!\n", __func__, ret);
			return ret;
		}
	}
...
static int pd_get_pdos(struct adapter_device *dev)
{
	...
	for (i = 0; i < 7; i++) {
		...
		adapter_dbg("[%s]: pdo[%d] { received_pdos is %08x, cap.pdos is %08x}\n",
...
APDO_REGAIN:
		...
		for (i = 0; i < pd_cap.nr; i++) {
			...
			adapter_dbg("[%s]:%d mv:[%d,%d] %d max:%d min:%d type:%d %d\n",
		...
		if (ret == TCPM_SUCCESS) {
			while (timeout < 10) {
				if (get_apdo_regain) {
					adapter_dbg("[%s] ready to get pps info!\n", __func__);
					goto APDO_REGAIN;
				} else {
					msleep(100);
					timeout++;
...
static int pd_get_usbpd_verifed(struct adapter_device *dev, bool *verifed)
{
	struct xm_pd_adapter_info *info;
	...
	*verifed = info->adapter_dev->verifed;
	adapter_dbg("[%s] current usbpd_verifed is %d\n", __func__, *verifed);
	return 0;
}
...
static int xm_pd_adapter_probe(struct platform_device *pdev)
{
	...
	info->adapter_dev = adapter_device_register(info->adapter_dev_name,
		&pdev->dev, info, &xm_adapter_ops, NULL);
