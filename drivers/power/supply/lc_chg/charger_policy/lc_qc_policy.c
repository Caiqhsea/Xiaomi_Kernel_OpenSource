...
#include "lc_qc_policy.h"

#define RETRY_CNT_MAX 100
...
static int vbus_low_gap = 150;
module_param_named(vbus_low_gap, vbus_low_gap, int, 0600);

static int vbus_high_gap = 500;
...
static void qcm_update_qc3_type(struct qcm_chip *chip)
{
	...
	charger_get_qc3_result(chip->charger, &qc3_res);
	if (chip->qc3_type) {
		chip->disable_slave = true;
		chip->max_step = QC3_MAX_TUNE_STEP;
		chip->tune_step_vbus = QC3_VBUS_STEP;
		if (qc3_res == QC3_5_27W_MODE) {
...
static int __init lc_qcm_init(void)
{
	printk("%s\n", __func__);
	return platform_driver_register(&qcm_driver);
}

static void __exit lc_qcm_exit(void)
{
	printk("%s\n", __func__);
	platform_driver_unregister(&qcm_driver);
}

module_init(lc_qcm_init);
module_exit(lc_qcm_exit);

MODULE_DESCRIPTION("LC driver");
