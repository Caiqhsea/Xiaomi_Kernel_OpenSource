...
static void xm_smart_chg_endurance_protect(struct charger_manager *manager)
{
	...
	lc_info("enter, en_ret = %d, fun_val = %d, active_status = %d, smart_soc = %d, ui_soc =%d, smart_ctrl_en = %d\n",
...
#if IS_ENABLED(CONFIG_XM_DFX_CHARGER)
	manager->endurance_changed_flag = (manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret == endurance_old_mode) ? false : true;
	lc_info("endurance changed flag %d, smart mode %d, endurance old mode %d", manager->endurance_changed_flag,
		manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret, endurance_old_mode);

	if (manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret != endurance_old_mode) {
		endurance_old_mode = manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret;
	}
#endif
...
#if IS_ENABLED(CONFIG_XM_DFX_CHARGER)
	if (manager->endurance_changed_flag) {
		if (manager->smart_charge[SMART_CHG_ENDURANCE_PRO].en_ret) {
			xm_handle_dfx_report(CHG_DFX_SMART_ENDURANCE_TRIGGERED, true);
			manager->ignore_upload_endurance_pro = manager->soc >= manager->smart_charge[SMART_CHG_ENDURANCE_PRO].func_val ? true : false;
		} else {
			xm_handle_dfx_report(CHG_DFX_SMART_ENDURANCE_TRIGGERED, false);
		}
	}

	if (manager->soc >= manager->smart_charge[SMART_CHG_ENDURANCE_PRO].func_val) {
		if (manager->soc >= manager->smart_charge[SMART_CHG_ENDURANCE_PRO].func_val + 3) {
			if (manager->smart_charge[SMART_CHG_ENDURANCE_PRO].active_status && !manager->ignore_upload_endurance_pro) {
				xm_handle_dfx_report(CHG_DFX_SMART_ENDURANCE_SOC_ERR, true);
				lc_info("endurance soc error\n");
				manager->ignore_upload_endurance_pro = true;
			}
		}
	}
#endif
...
#ifdef CONFIG_LC_CP_POLICY
	lc_info("monitor_smart_chg, en_ret = %d, fun_val = %d, active_status = %d, smart_soc = %d, ui_soc =%d, smart_ctrl_en = %d, state = %d\n",
#else
	lc_info("monitor_smart_chg, en_ret = %d, fun_val = %d, active_status = %d, smart_soc = %d, ui_soc =%d, smart_ctrl_en = %d\n",
#endif
...
#endif
#if IS_ENABLED(CONFIG_XM_DFX_CHARGER)
	manager->navigation_changed_flag = (manager->smart_charge[SMART_CHG_NAVIGATION].en_ret == navigation_old_mode) ? false : true;
	lc_info("navigation changed flag %d, smart mode %d, navigation old mode %d", manager->navigation_changed_flag,
			manager->smart_charge[SMART_CHG_NAVIGATION].en_ret, navigation_old_mode);
	if (manager->smart_charge[SMART_CHG_NAVIGATION].en_ret != navigation_old_mode) {
		navigation_old_mode = manager->smart_charge[SMART_CHG_NAVIGATION].en_ret;
	}
#endif
	if (manager->smart_charge[SMART_CHG_NAVIGATION].en_ret &&
			manager->soc >= manager->smart_charge[SMART_CHG_NAVIGATION].func_val &&
			(!manager->smart_charge[SMART_CHG_NAVIGATION].active_status)) {
		...
		if (g_policy->state == POLICY_RUNNING) {
			chargerpump_policy_stop(g_policy);
			lc_info("monitor disable cp\n");
		}
#endif
		manager->smart_ctrl_en = true;
		lc_info("monitor disable charger,soc >= %d\n", manager->smart_charge[SMART_CHG_NAVIGATION].func_val);
	} else if((((!manager->smart_charge[SMART_CHG_NAVIGATION].en_ret ||
				manager->soc <= manager->smart_charge[SMART_CHG_NAVIGATION].func_val - 5) &&
				manager->smart_charge[SMART_CHG_NAVIGATION].active_status) ||
				(!manager->smart_charge[SMART_CHG_NAVIGATION].en_ret &&
				!manager->smart_charge[SMART_CHG_NAVIGATION].active_status)) && manager->smart_ctrl_en) {
...
#endif
		lc_info("monitor enable charger, soc <= %d\n", manager->smart_charge[SMART_CHG_NAVIGATION].func_val - 5);
	}
#if IS_ENABLED(CONFIG_XM_DFX_CHARGER)
	if (manager->navigation_changed_flag) {
		if (manager->smart_charge[SMART_CHG_NAVIGATION].en_ret) {
			xm_handle_dfx_report(CHG_DFX_NAVIGATION, true);
			manager->ignore_upload_navigation = manager->soc >= manager->smart_charge[SMART_CHG_NAVIGATION].func_val ? true : false;
		} else {
			xm_handle_dfx_report(CHG_DFX_NAVIGATION, false);
		}
	}
	if (manager->soc >= manager->smart_charge[SMART_CHG_NAVIGATION].func_val) {
		if (manager->soc >= manager->smart_charge[SMART_CHG_NAVIGATION].func_val + 3) {
			if (manager->smart_charge[SMART_CHG_NAVIGATION].active_status && !manager->ignore_upload_navigation) {
				xm_handle_dfx_report(CHG_DFX_NAVIGATION_OVER_SOC, true);
				lc_info("navigation soc error\n");
				manager->ignore_upload_navigation = true;
			}
		}
	}
#endif
...
#else
	if ((manager == NULL) || !manager->main_chg_disable_votable)
#endif
	return;

	lc_info("%s night_charging = %d, soc = %d\n", __func__, manager->night_charging, manager->soc);
...
void monitor_low_fast_strategy(struct charger_manager *manager)
{
	...
	lc_err("%s soc = %d, thermal_level = %d, thermal_board_temp = %d, pd_active = %d, low_fast_plugin_flag = %d, \
			low_fast_enable = %d, screen_state = %d, b_flag = %d\n", __func__, manager->soc, manager->system_temp_level,
		manager->thermal_board_temp, manager->pd_active, manager->low_fast_plugin_flag,
        manager->smart_charge[SMART_CHG_LOW_FAST].active_status, manager->sm.screen_state, manager->b_flag);

	if ((manager->pd_active == CHARGE_PD_PPS_ACTIVE) && (manager->soc <= 40) && (manager->low_fast_plugin_flag) &&
		...
		} else if ((last_level == 7) && ((manager->system_temp_level == 7) || (manager->system_temp_level == 8)) &&
        			hot_flag && (manager->thermal_board_temp > 410)){
			fast_flag = false;
		} else {
			hot_flag = false;
		}
		...
		if (fast_flag) {  //stay fast strategy
			...
			if ((manager->soc > 38) && (manager->thermal_board_temp > 380)) {
				...
				lc_err("%s stay fast but decrease 1, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", __func__,
					manager->thermal_board_temp, manager->low_fast_ffc);
...
void monitor_reset_vote(struct charger_manager *manager)
{
	if (IS_ERR_OR_NULL(manager)){
		lc_err("manager is NULL\n");
		return;
...
void xm_charge_work(struct work_struct *work)
{
	...
	* */
	...
	schedule_delayed_work(&chip->xm_charge_work, msecs_to_jiffies(10000));
}
