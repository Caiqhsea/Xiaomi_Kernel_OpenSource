    ...
    struct class_attribute *attr, char *buf)
{
	...
	battery_id = fuel_gauge_get_battery_id(manager->fuel_gauge);
out:
	return sprintf(buf, "%s\n", r_items_param_fg[battery_id].manufacturer);
}
	...
	struct class_attribute *attr, char *buf)
{
	...
	battery_id = fuel_gauge_get_battery_id(manager->fuel_gauge);
out:
	return sprintf(buf, "%d\n", r_items_param_fg[battery_id].bat_resistance_id);
}
        ...
        struct class_attribute *attr, char *buf)
{
	...
	battery_id = fuel_gauge_get_battery_id(manager->fuel_gauge);
out:
	return sprintf(buf, "%s\n", r_items_param_fg[battery_id].batt_type);
}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	ret = fuel_gauge_set_shipmode(manager->fuel_gauge, manager->shippingmode);
	if (ret < 0)
		lc_err("fg can not get shipmode\n");
				...
				struct class_attribute *attr, char *buf)
{
	...
	if(rc < 0){
		lc_err("get eu_mode from charger parition failed, ret = %d\n", rc);
		return -EINVAL;
	}
	lc_err("eu_mode_val: %d \n", val);
	return scnprintf(buf, PAGE_SIZE, "%d\n", val);
}
				...
				struct class_attribute *attr,const char *buf, size_t len)
{
	...
	if (rc) {
		lc_err("kstrtoint fail\n");
		return -EINVAL;
	}
	rc = charger_partition_set_prop(CHARGER_PARTITION_PROP_EU_MODE, val);
	if(rc < 0){
		lc_err("set eu_mode to charger parition failed, ret = %d\n", rc);
		return -EINVAL;
	}
	return len;
}
...
static ssize_t rsoc_show(struct class *c,
		struct class_attribute *attr, char *buf)
{
	struct power_supply *psy_bms = NULL;
	struct sm_fg_chip *gm;
		...
		struct class_attribute *attr, char *buf)
{
	struct charger_manager *manager = container_of(c,
			struct charger_manager, lc_charger_class);
	lc_err("%s: shutdown_delay: %d\n", __func__, manager->shutdown_delay);
	if (IS_ERR_OR_NULL(manager))
		return sprintf(buf, "%d\n", 0);
	else
...
static ssize_t sw_manufacturer_show(struct class *c,
    struct class_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", "Southchip-SC6601");
}
...
static int get_apdo_max(struct charger_manager *manager) {
	...
	ret = adapter_get_cap(adapter, &pd_cap);
		if (ret < 0) {
			lc_info("adapter get cap failed\n");
			goto done;
		}
	apdo_max = pd_cap.volt_max[pd_cap.cnt] *
		pd_cap.curr_max[pd_cap.cnt] / 1000000;
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	lc_err("audio_cctog %d \n", manager->audio_cctog);
	// otg switch off
	if (!manager->ui_cc_toggle) {
		if (!manager->swcid_bright && !manager->typec_attach) {
			if (!manager->audio_cctog){
				lc_info("audio off, set snk\n");
				tcpm_typec_change_role_postpone(manager->tcpc, TYPEC_ROLE_SNK, true);
			} else {
				lc_info("audio on, try snk\n");
...
//battery secret ic
static ssize_t soh_sn_show(struct class *dev,
	struct class_attribute *attr, char *buf)
{
	const char *soh_sn;
...
static ssize_t manufacturing_date_show(struct class *dev,
	struct class_attribute *attr, char *buf)
{
	...
	if (manufacturing_date) {
		lc_err("get battery_manufacture_date succ\n");
		return sprintf(buf, "%s\n", manufacturing_date);
	} else {
		lc_err("get battery_manufacture_date fail\n");
...
static ssize_t first_usage_date_show(struct class *dev,
	struct class_attribute *attr, char *buf)
{
...
static ssize_t first_usage_date_store(struct class *dev,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	char first_usage_date[7] = {0};

	lc_err("first_usage_date:%s\n", buf);
...
static ssize_t reset_cycle_store(struct class *dev,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	...
	if (ret == 0) {
		ret = lc_clear_cycle_count();
			if (ret == 0) {
				lc_err("reset_cycle succ\n");
				return count;
			} else {
				lc_err("reset_cycle fail\n");
				return -EINVAL;
			}
	} else {
		lc_err("reset_cycle_key error=%d\n", ret);
		return -EINVAL;
	}
}
	...
	struct class_attribute *attr, char *buf)
{
	...
	if (power_supply_get_property(batt_psy, POWER_SUPPLY_PROP_CYCLE_COUNT, &pval)) {
		lc_err("get sm5602 fgu cycle count failed\n");
		return -EINVAL;
	}

	return scnprintf(buf, PAGE_SIZE, "%d\n", pval.intval);
}
static CLASS_ATTR_RO(cycle_count);

static ssize_t soh_new_show(struct class *dev,
        struct class_attribute *attr, char *buf)
{
	union power_supply_propval pval;
...
static ssize_t ui_soh_show(struct class *dev,
	struct class_attribute *attr, char *buf)
{
	int ret = 0;
	u8  ui_soh_data[16] = {0};
...
static ssize_t ui_soh_store(struct class *dev,
	struct class_attribute *attr, const char *buf, size_t count)
{
	char t_data[70] = {0};
		...
		struct class_attribute *attr, char *buf)
{
	...
	sm = fuel_gauge_get_private(manager->fuel_gauge);
	if (IS_ERR_OR_NULL(sm)) {
		lc_err("Failed to get sm_fg_chip data\n");
		return PTR_ERR(sm);
	}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoint(buf, 10, &val)) {
		lc_err("Invalid input format\n");
		return -EINVAL;
	}

	if (val < 0) {
		lc_err("Negative values not allowed: %d\n", val);
		return -EINVAL;
	}

	sm->fake_cycle_count = val;
	lc_info("new fake_cycle_count:%d\n", val);
...
int lc_charger_node_init(struct charger_manager *manager)
{
	...
	manager->lc_charger_class.name = "qcom-battery";
