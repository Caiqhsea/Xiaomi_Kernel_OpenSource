...
static int chargerpump_policy_state_init(struct chargerpump_policy *policy)
{
	...
	|| (policy->decrease_volt == 1)
		...
		policy->sm = PM_STATE_CHARGERPUMP_RESTART;
	} else {
...
static int __chargerpump_cc_cv_algo(struct chargerpump_policy *policy)
{
	...
	sm = fuel_gauge_get_private(policy->fuel_gauge);
	...
	if ((info->vbat > (final_cv - cfg->cv_offset)) && ((info->ibat < cfg->exit_cc) || (sm->rsoc >= 99))) {
		if (iterm_cnt++ > CHARGERPUMP_POLICY_END_CHG_CNT) {
			vote(policy->total_fcc_votable, MAIN_FCC_TAPER_VOTER, true, max((int)(info->v_iterm + 100), info->ibat));
	...
	while (true) {
		...
		switch (policy->sm) {
			...
			case PM_STATE_CHARGERPUMP_CC_CV:
				...
				break;
			case PM_STATE_CHARGERPUMP_RESTART:
				dv_policy_state_chargerpump_exit(policy);
				policy->state = POLICY_NO_START;
				break;
...
int chargerpump_policy_start(struct chargerpump_policy *policy)
{
	...
	if (info->total_fcc > 0 && info->total_fcc < cfg->exit_cc) {
		lc_err("total_fcc(%d) is lower than exit_cc(%d), use main charging\n", info->total_fcc, cfg->exit_cc);
		return 0;
	}
