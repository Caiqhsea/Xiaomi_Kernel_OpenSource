...
enum xm_property_id {
	...
	/* Add for xiaomi PD adapter auth */
	...
	XM_POWER_SUPPLY_PROP_CYCLE_FAKE,
	XM_POWER_SUPPLY_PROP_TEMP_FAKE,
	XM_PROP_MTBF_CURRENT,
	XM_POWER_SUPPLY_PROP_RAW_SOC,
	XM_POWER_SUPPLY_PROP_CONNECTOR_TEMP,
	XM_POWER_SUPPLY_PROP_SMART_BATT,
	XM_POWER_SUPPLY_PROP_NIGHT_CHARGING,
...
enum {
	...
	QTI_POWER_SUPPLY_USB_TYPE_HVDCP_3_CLASSB=0x84,
};
...
struct battery_chg_dev {
	...
	struct power_supply		*auth_psy;
	void				*notifier_cookie;
	/* extcon for VBUS/ID notification for USB for micro USB */
	struct extcon_dev		*extcon;
        u32				*ss_auth_data; // Add for xiaomi PD adapter auth
	u32				*thermal_levels;
...
static const char * const power_supply_battery_id_text[] = {
	"Unknown", "XWD", "NVT"
};
...
static const char * const qc_power_supply_usb_type_text[] = {
	"HVDCP", "HVDCP_3", "HVDCP_3P5", "FLOAT", "HVDCP_3"
};
				...
				size_t len)
{
	...
	struct battery_model_resp_msg *model_resp_msg = data;
        struct xm_ss_auth_resp_msg *ss_auth_req_msg = data; // Add for xm PD adapter auth
...
static u8 last_quick_charge_type = QUICK_CHARGE_NORMAL;
int get_quick_charge_type(struct battery_chg_dev *bcdev, union power_supply_propval *val)
{
	...
	rc = read_property_id(bcdev, batt_pst, BATT_TEMP);
	if (rc < 0)
		return rc;
	...
	pr_err("usb_real_type[%d][%s] \n", pst->prop[USB_REAL_TYPE], get_usb_type_name(pst->prop[USB_REAL_TYPE]));

	if (batt_temp < 500 || batt_temp >= 4800) {
		...
		union power_supply_propval *pval)
{
	...
	if (prop == POWER_SUPPLY_PROP_ONLINE) {
		...
		if(val.intval != last_quick_charge_type){
			if(pval->intval != last_usb_online || pval->intval == 1){
				pr_err("quick_charge_type(%d), last_quick_charge_type(%d), usb_online(%d), last_usb_online(%d) \n", val.intval, last_quick_charge_type, pval->intval, last_usb_online);
				if(IS_ERR_OR_NULL(pst) || IS_ERR_OR_NULL(pst->psy)){
					pr_err("Failed to get the pst ! \n");
				}else{
					power_supply_changed(pst->psy);
        			}

				if(IS_ERR_OR_NULL(batt_pst) || IS_ERR_OR_NULL(batt_pst->psy)){
					pr_err("Failed to get the batt_pst ! \n");
				}else{
			...
			last_quick_charge_type = val.intval;
		}
	}
...
#define FAKE_TMEP_INIT  -999
#define CAPACITY_LEVEL_CRITICAL_VOLT 3250000
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (bcdev->fake_temp != FAKE_TMEP_INIT)
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		...
		if (rc < 0){
			...
			pr_err("chj, raw_soc is %d, last_raw_soc is %d, soc_reduce is %d \n", raw_soc, last_raw_soc, soc_reduce);
		...
		if(ui_soc <= 100 && raw_soc >= 0 && soc_reduce == true){
			...
			char *buf)
{
	...
	if (usb_present && pst->prop[XM_PROP_REAL_TYPE] == 0){
		pst->prop[XM_PROP_REAL_TYPE] = 13;
    }

	pr_err("usb_online = %d, real_type[%d][%s]\n", usb_present,
			pst->prop[XM_PROP_REAL_TYPE], power_supply_usb_type_text[pst->prop[XM_PROP_REAL_TYPE]]);

	return scnprintf(buf, PAGE_SIZE, "%s\n", power_supply_usb_type_text[pst->prop[XM_PROP_REAL_TYPE]]);
}
static CLASS_ATTR_RO(real_type);

static ssize_t usb_present_show(struct class *c,
					struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
					...
					struct class_attribute *attr, char *buf)
{
	...
	rc = read_property_id(bcdev, pst, XM_PROP_BATTERY_ID);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%s\n", power_supply_battery_id_text[pst->prop[XM_PROP_BATTERY_ID]]);
}
					...
					struct class_attribute *attr, char *buf)
{
	...
	if (pst->prop[XM_PROP_BATTERY_ID] == 1)
		return scnprintf(buf, PAGE_SIZE, "XWD-10000mAh\n");
	else if (pst->prop[XM_PROP_BATTERY_ID] == 2)
		return scnprintf(buf, PAGE_SIZE, "NVT-10000mAh\n");
	else
...
/* HTH-378562 add input_suspend start */
...
/* HTH-378562 add input_suspend end */

static ssize_t fast_chg_mode_show(struct class *c,
					struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
...
static bool is_auth_psy_available(struct battery_chg_dev *bcdev)
{
	if (!bcdev->auth_psy)
		bcdev->auth_psy = power_supply_get_by_name("batt_verify");
					...
					struct class_attribute *attr, char *buf)
{
	...
	if (!is_auth_psy_available(bcdev))
		return -ENODEV;

	rc = power_supply_get_property(bcdev->auth_psy, POWER_SUPPLY_PROP_ONLINE, &pval);
	if (rc < 0) {
		pr_err("Couldn't get chip_ok ret=%d\n", rc);
	}
					...
					const char *buf, size_t count)
{
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM], XM_POWER_SUPPLY_PROP_CYCLE_FAKE, val);
	if (rc < 0)
		return rc;
					...
					const char *buf, size_t count)
{
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM], XM_POWER_SUPPLY_PROP_TEMP_FAKE, val);
	if (rc < 0)
		return rc;
				...
				char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%u", soc);
}
									...
									const char *buf, size_t count)
{
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM], XM_POWER_SUPPLY_PROP_CONNECTOR_TEMP, val);
	if (rc < 0)
		return rc;
	bcdev->fake_connector_temp = val;

	return count;
}

static ssize_t connector_temp_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	...
	rc = read_property_id(bcdev, pst, XM_POWER_SUPPLY_PROP_CONNECTOR_TEMP);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%d", (int)pst->prop[XM_POWER_SUPPLY_PROP_CONNECTOR_TEMP]);
}
...
static ssize_t night_charging_store(struct class *c, struct class_attribute *attr,
						const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_XM], XM_POWER_SUPPLY_PROP_NIGHT_CHARGING, val);
	if (rc < 0)
		return rc;
...
static ssize_t night_charging_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
	...
	rc = read_property_id(bcdev, pst, XM_POWER_SUPPLY_PROP_NIGHT_CHARGING);
	if (rc < 0)
		return rc;

	return scnprintf(buf, PAGE_SIZE, "%d", pst->prop[XM_POWER_SUPPLY_PROP_NIGHT_CHARGING]);
}
...
static struct attribute *chg_dbg_attrs[] = {
	NULL,
};
ATTRIBUTE_GROUPS(chg_dbg);
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	bcdev->fake_temp = FAKE_TMEP_INIT;
	bcdev->fake_connector_temp = -EINVAL;
	...
	bcdev->chg_dbg_class.name = "chg-dbg";
	bcdev->chg_dbg_class.class_groups = chg_dbg_groups;
	rc = class_register(&bcdev->chg_dbg_class);
	if (rc < 0) {
		dev_err(dev, "Failed to create chg_dbg_class rc=%d\n", rc);
	}
...
static int battery_chg_remove(struct platform_device *pdev)
{
	...
	class_unregister(&bcdev->chg_dbg_class);
	pmic_glink_unregister_client(bcdev->client);
	cancel_work_sync(&bcdev->subsys_up_work);
	cancel_work_sync(&bcdev->usb_type_work);
	cancel_work_sync(&bcdev->battery_check_work);
