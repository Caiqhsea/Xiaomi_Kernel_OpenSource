...
static atomic_t g_camera_reserved_pages = ATOMIC_INIT(0); //min reserve pages for camera
static atomic_t g_camera_provider_pid = ATOMIC_INIT(-1); // camera provider pid
static atomic_t g_allocator_pid = ATOMIC_INIT(-1);  // allocator-servi pid
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < POOL_PROCESS_NUMBER; i++) {
		if (len < PAGE_SIZE - 1)
			len += snprintf(buf + len, PAGE_SIZE - len, "%d ", pool_pid[i]);
	}

	if (len < PAGE_SIZE - 1)
		len += snprintf(buf + len, PAGE_SIZE - len, "cmp:%d\n",
				use_name_cmp);

	return len;
}

static ssize_t cam_reserved_pages_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int len = snprintf(buf, PAGE_SIZE, "%d ", atomic_read(&g_camera_reserved_pages));
	return len;
}

static ssize_t cam_reserved_pages_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	int _camera_reserved_pages, ret;

	ret = kstrtoint(buf, 0, &_camera_reserved_pages);
	if (ret){
		pr_err("%s read extrapages error", __func__);
		return ret;
	}

	atomic_set(&g_camera_reserved_pages, _camera_reserved_pages);
	return len;
}
...
static bool update_pool_pid(int new_pid)
{
	...
	for (i = 0; i < POOL_PROCESS_NUMBER; i++) {
		...
		if (pid < 0 || !find_task_by_vpid(pid)) {
			...
			atomic_set(&g_camera_provider_pid, -1);
			atomic_set(&g_allocator_pid, -1);
			break;
		}
		rcu_read_unlock();
	}
...
static RESERVE_POOL_ATTR(cam_reserved_pages, 0660, cam_reserved_pages_show, cam_reserved_pages_store);
...
static struct attribute *reserve_pool_attrs[] = {
	...
	&kobj_attr_cam_reserved_pages.attr,
	NULL,
};
...
static void clear_pid_from_pool_pid(void *unused1, void *unused2)
{
	...
	if (!g_sys_heap->reserve_extend_info.use_reserve_pool ||
...
static void update_camera_proc_info(struct task_struct *task)
{
	char buf[128] = {0};
	...
	/*
	 * Use strncpy_from_user() instead of strnlen_user() + copy_from_user()
	 * to avoid TOCTOU race condition and fix undefined symbol error.
	 *
	 * Note: strncpy_from_user() returns length WITHOUT the NUL terminator,
	 * while strnlen_user() returns length WITH the NUL terminator.
	 * We need to advance start by ret + 1 to skip past the NUL in userspace.
	 */
	while (start < end && len < buf_len - 1) {
		unsigned long remaining = end - start;
		int space_left = buf_len - 1 - len;

		if (remaining > space_left)
			remaining = space_left;

		ret = strncpy_from_user(buf + len, (char __user *)start, remaining);
		if (ret < 0)
			break;

		len += ret;
		start += ret + 1;  /* Skip past the NUL terminator in userspace */

		/* Add separator between arguments */
		if (start < end && len < buf_len - 1) {
			buf[len++] = '\0';
		}
	}
	...
	if (strstr(buf, served_process[0]) != NULL) {
		atomic_set(&g_camera_provider_pid, current->group_leader->pid);
	} else if (strstr(buf, served_process[1]) != NULL) {
		atomic_set(&g_allocator_pid, current->group_leader->pid);
	}
	mmput(mm);
}

static bool is_served_app(struct dynamic_page_pool **reserve_pools) {
	get_task_struct(current);
	if (reserve_pools != NULL && current->signal->oom_score_adj == NATIVE_ADJ) {
		int j;

		if(atomic_read(&g_camera_provider_pid) <= 0 || atomic_read(&g_allocator_pid) <= 0) {
			update_camera_proc_info(current->group_leader);
		}
		...
		struct dynamic_page_pool *spool, struct dynamic_page_pool *reserve_pool)
{
	...
	int camera_reserved_pages = atomic_read(&g_camera_reserved_pages);

	if ((reserve_pool->order == 0) && (camera_reserved_pages > 0 )) {
		unsigned long order0_pool_free_size = atomic_read(&(reserve_pool->count));
		int camera_provider_pid = atomic_read(&g_camera_provider_pid);
		int allocator_pid = atomic_read(&g_allocator_pid);
		if (sys_heap->reserve_extend_info.debug & RESERVE_POOL) {
			//for debug
			pr_info("%s: camera_reserved_pages: %d, scene %d, order0_pool_free_size: %lu, pid :%d, camerapid: %d, allocator_pid %d\n",
					__func__, camera_reserved_pages, g_sys_heap->reserve_extend_info.scene,
					order0_pool_free_size, current->group_leader->pid, camera_provider_pid, allocator_pid);
		}

		if(order0_pool_free_size <= camera_reserved_pages) {
			//if the remaining pages is below camera reserved pages
			bool isAllow = false;
			if(camera_provider_pid > 0 &&
					current->group_leader->pid == camera_provider_pid) {
				isAllow = true;
			} else if(allocator_pid > 0 &&
					g_sys_heap->reserve_extend_info.scene == CAMERA_FG &&
					current->group_leader->pid == allocator_pid) {
				isAllow = true;
			}
			if(!isAllow) {
				pr_info("%s: not allow used reserve pool", __func__);
				return NULL;
			}
		}
	}
						   ...
						   bool movable)
{
	...
	bool served_app = is_served_app(reserve_pools);
	int i;
