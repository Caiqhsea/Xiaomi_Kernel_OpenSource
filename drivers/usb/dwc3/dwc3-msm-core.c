...
int usb_send_headset_msg(struct usb_device *udev)
{
...
static void gsi_configure_ep(struct usb_ep *ep, struct usb_gsi_request *request)
{
	...
	*/
	...
	dwc3_core_send_gadget_ep_cmd(dep, DWC3_DEPCMD_SETTRANSFRESOURCE, &params);
...
struct mca_sysfs_attr_info dwc3_msm_sysfs_field_tbl[] = {
	...
	mca_sysfs_attr_rw(dwc3_msm_sysfs, 0664, DWC3_MSM_PROP_AUDIO_MULTI_CHANN_SUPPORT, audio_multi_chann_support),
};
                ...
                struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case DWC3_MSM_PROP_AUDIO_MULTI_CHANN_SUPPORT:
		...
		pr_err("%s: audio_multi_chann_support is %d\n", __func__, val);
		break;
	default:
		break;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	case DWC3_MSM_PROP_SUPER_SPEED:
		...
		break;
	case DWC3_MSM_PROP_AUDIO_MULTI_CHANN_SUPPORT:
		ret = g_mdwc->audio_multi_chann_support;
		count = scnprintf(buf, PAGE_SIZE, "%s\n", ret ? "true" : "false");
		pr_err("%s: audio_multi_chann_support is %d\n", __func__, ret);
		break;
	default:
...
typedef enum {
	WCD_HARDWARE_PROJECT_A = 5,
	WCD_HARDWARE_PROJECT_B = 7,
	WCD_HARDWARE_PROJECT_C = 8,
	WCD_HARDWARE_PROJECT_D = 9,
	WCD_HARDWARE_PROJECT_E = 10,
	WCD_HARDWARE_PROJECT_F = 11,
	WCD_HARDWARE_PROJECT_G = 12
} Platform_Num_Dis_Wcd;

static const int Platform_Num_Dis_Wcd_Array[] = {
	WCD_HARDWARE_PROJECT_A,
	WCD_HARDWARE_PROJECT_B,
	WCD_HARDWARE_PROJECT_C,
	WCD_HARDWARE_PROJECT_D,
	WCD_HARDWARE_PROJECT_E,
	WCD_HARDWARE_PROJECT_F,
	WCD_HARDWARE_PROJECT_G
};
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	for(;j < (sizeof(Platform_Num_Dis_Wcd_Array) / sizeof(Platform_Num_Dis_Wcd_Array[0]));j++)
		if(platform_id == Platform_Num_Dis_Wcd_Array[j])
			break;

	if(j == (sizeof(Platform_Num_Dis_Wcd_Array)/sizeof(Platform_Num_Dis_Wcd_Array[0]))) {
			...
			pr_info("platform_id = %d,wcd load success\n",platform_id);
	}
	else
		pr_info("platform_id = %d,wcd disbale\n",platform_id);
	/* Assumes dwc3 is the first DT child of dwc3-msm */
	dwc3_node = of_get_next_available_child(node, NULL);
	if (!dwc3_node) {
...
static void dwc3_msm_update_interfaces(struct usb_device *udev)
{
	...
	if (udev->descriptor.idVendor == 0x2717 && udev->descriptor.idProduct == 0x50d3) {
		if (g_mdwc->audio_multi_chann_support) {
			dev_info(&udev->dev, "device and headset all support multi channels\n");
