...
struct tcpc_ops {
	...
	int (*get_fake_power_status)(struct tcpc_device *tcpc, uint16_t *pwr_status); //add by SY
	...
	int (*pd_enable)(struct tcpc_device *tcpc, int enable);
	...
	int (*force_retry)(struct tcpc_device *tcpc);//uu635
	int (*set_bg_enable)(struct tcpc_device *tcpc, bool enable); //add by SY
...
struct tcpc_device {
	...
	int get_source_cap_flag;
	int retry_time_flag;
	/* For TCPC TypeC */
	uint8_t flag_rx_one; //uu668
	uint8_t flag_tx_one; //uu668
	uint8_t g_buf_ba41_rx[33];//uu668
