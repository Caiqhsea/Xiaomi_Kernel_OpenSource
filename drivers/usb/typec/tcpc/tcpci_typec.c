...
#if IS_ENABLED(CONFIG_QCOM_AUDIO_PINCTRL)
extern int audio_pinctrl_update(bool active);
#endif /* CONFIG_QCOM_AUDIO_PINCTRL */
...
int tcpc_typec_handle_timeout(struct tcpc_device *tcpc, uint32_t timer_id)
{
    ...
    case TYPEC_TIMER_DRP_SRC_TOGGLE:
        ...
        break;
    case TYPEC_TIMER_GET_SRC_HDR:
        tcpc->get_source_cap_flag = 1;
        tcpc_disable_timer(tcpc,TYPEC_TIMER_GET_SRC_HDR);
        break;

    // case TYPEC_TIMER_RETRY_TIME:
    //     tcpc->retry_time_flag = 1;
        // tcpc_disable_timer(tcpc,TYPEC_TIMER_RETRY_TIME);
        // break;
