...
static void tcp_notify_func(struct work_struct *work)
{
    ...
    timeval = (end - begin) / NSEC_PER_MSEC;
...
int tcpci_get_fake_power_status(struct tcpc_device *tcpc, uint16_t *pw_status)
{
	...
	if (tcpc->ops->get_fake_power_status)
...
{
	PD_BUG_ON(tcpc->ops->pd_enable == NULL);

	return tcpc->ops->pd_enable(tcpc, enable);
}
...
//uu635
int tcpci_retry(struct tcpc_device *tcpc)
{
	PD_BUG_ON(tcpc->ops->force_retry == NULL);

	return tcpc->ops->force_retry(tcpc);
}
