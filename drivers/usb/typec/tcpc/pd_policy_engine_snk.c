                ...
                struct pd_port *pd_port)
{
    ...
    /* typec current > sourcecap current */
    /*
    if (pd_check_pe_during_hard_reset(pd_port))
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
    tcpc_typec_handle_cc_change(pd_port->tcpc);
#endif	/* CONFIG_SUPPORT_SOUTHCHIP_PDPHY */
...
void pe_snk_evaluate_capability_entry(struct pd_port *pd_port)
{
    ...
    if ((pd_port->dpm_charging_policy & DPM_CHARGING_POLICY_MASK) < DPM_CHARGING_POLICY_PPS) {
...
void pe_snk_select_capability_entry(struct pd_port *pd_port)
{
    ...
    if ((NU6601_DID_A == chip_id) && (pd_port->pd_connect_state != PD_CONNECT_PE_READY_SNK_APDO)) {
        if (pd_port->tcpc->pd_wait_pr_swap_complete == false) {
            if ((0x0 == RDO_FIXED_VAR_EXTRACT_OP_CURR(pd_port->last_rdo))) {
                tcpci_sink_vbus(pd_port->tcpc,
                        TCP_VBUS_CTRL_REQUEST, TCPC_VBUS_SINK_5V, 0);
            }
        }
    }
...
void pe_snk_ready_entry(struct pd_port *pd_port)
{
    ...
    if (pd_port->tcpc) {
		tcpci_get_chip_id(pd_port->tcpc,&chip_id);
		if (chip_id == SY6976_DID_A) {
			tcpci_set_bg_enable(pd_port->tcpc,0);
		}
	}
}
