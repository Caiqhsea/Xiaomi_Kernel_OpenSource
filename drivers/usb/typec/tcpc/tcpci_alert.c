...
static int tcpci_vbus_level_changed(struct tcpc_device *tcpc)
{
    ...
    if (chip_id == NU6601_DID_A)
    {
        if((tcpc->vbus_level == TCPC_VBUS_VALID) && (tcpc->typec_state == 7))
        {
...
static int tcpci_alert_power_status_changed(struct tcpc_device *tcpc)
{
	...
	if (chip_id == SY6976_DID_A)
		rv = tcpci_get_fake_power_status(tcpc, &power_status);
	else
...
int tcpci_alert_fake_power_status_changed(struct tcpc_device *tcpc)
{
	int rv = 0;
	uint16_t power_status = 0;
	rv = tcpci_get_power_status(tcpc, &power_status);
...
EXPORT_SYMBOL(tcpci_alert_fake_power_status_changed);
...
int tcpci_alert(struct tcpc_device *tcpc)
{
    ...
    uint32_t chip_vid = 0;
#endif /* CONFIG_SUPPORT_SOUTHCHIP_PDPHY */
    // if(1 == tcpc->retry_time_flag){
    //     tcpc->retry_time_flag = 0;
    //     tcpci_alert_status_clear(tcpc,TCPC_REG_ALERT_TX_FAILED);
    // }
