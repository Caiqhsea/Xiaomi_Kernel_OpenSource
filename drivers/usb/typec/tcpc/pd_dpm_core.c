...
#define NORMAL_SRC_CAP         0x260190c8 //5V2A
#define HIGH_SOC_SRC_CAP        0x26019096 //5V 1.5A
#define MID_SOC_SRC_CAP        0x26019064 //5V1A
#define LOW_SOC_SRC_CAP        0x26019032 //5V0.5A
#define LOW_TEMP_SRC_CAP        0x26019014 //5V 0.2A
#define HIGH_CYCLE_LOW_TEMP_SRC_CAP       0x2601900A //5V0.1A
int pd_dpm_send_source_caps(struct pd_port *pd_port)
{
    ...
    struct pd_port_power_caps *src_cap2 = &pd_port->local_src_cap_rc_18w;
    union power_supply_propval soc_val, temp_val;// cycle_val;
    ...
    if (bat_psy) {
        power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_CAPACITY, &soc_val);
        power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_TEMP, &temp_val);
        //power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_CYCLE_COUNT, &cycle_val);
    }

    if (soc_val.intval > 80) {
        src_cap0->pdos[0] = NORMAL_SRC_CAP;
    } else if (soc_val.intval > 60 && soc_val.intval <= 80) {
        src_cap0->pdos[0] = HIGH_SOC_SRC_CAP;
    } else if (soc_val.intval > 40 && soc_val.intval <= 60) {
        src_cap0->pdos[0] = MID_SOC_SRC_CAP;
    } else if (soc_val.intval > 15 && soc_val.intval <= 40) {
        src_cap0->pdos[0] = LOW_SOC_SRC_CAP;
    } else {
        src_cap0->pdos[0] = LOW_TEMP_SRC_CAP;
    }

    if (temp_val.intval <= 0) {
        src_cap0->pdos[0] = HIGH_CYCLE_LOW_TEMP_SRC_CAP;
    }
    ...
    if (pd_port->gear == 9000) {
        src_cap1->nr = src_cap2->nr;
        for (i = 0; i < src_cap2->nr; i++) {
            src_cap1->pdos[i] =
                pd_reset_pdo_power(tcpc, src_cap2->pdos[i], cable_curr);
        }
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
int pd_dpm_send_revison(struct pd_port *pd_port)
{
    uint32_t chip_id = 0;
    uint32_t verdo = SOUTHCHIP_PD_VER << 16;

    tcpci_get_chip_id(pd_port->tcpc,&chip_id);
    if(NU6601_DID_A == chip_id) {
        verdo = NUVOLTA_PD_VER << 16;
    }

    return pd_send_sop_data_msg(pd_port,
        PD_DATA_GET_REVISION, PD_VERDO_SIZE, &verdo);
}
#endif /* CONFIG_SUPPORT_SOUTHCHIP_PDPHY */
