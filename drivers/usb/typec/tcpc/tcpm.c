...
int tcpm_typec_re_release_gear(struct tcpc_device *tcpc, int gear)
{
    int ret = 0;
    struct pd_port *pd_port = &tcpc->pd_port;
    ...
    ret = pd_put_source_cap_event(pd_port);

    return ret;
}
EXPORT_SYMBOL(tcpm_typec_re_release_gear);
    ...
    uint32_t tout_ms, uint8_t *data, uint8_t size)
{
    ...
    do {
        ret = __tcpm_put_tcp_dpm_event_bk(
            tcpc, event, tout_ms, data, size);
    } while ((ret == TCP_DPM_RET_TIMEOUT || ret == TCP_DPM_RET_BK_TIMEOUT) && (retry-- > 0));
