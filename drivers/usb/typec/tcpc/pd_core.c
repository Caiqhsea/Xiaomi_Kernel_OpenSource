...
static int pd_parse_pdata(struct pd_port *pd_port)
{
    ...
    if (np) {
        ...
        ret = of_property_read_u32(np, "pd,source-18w-pdo-size",
                (u32 *)&pd_port->local_src_cap_rc_18w.nr);
        if (ret < 0)
            pr_err("%s get source 18w pdo size fail\n", __func__);

        ret = of_property_read_u32_array(np, "pd,source-18w-pdo-data",
            (u32 *)pd_port->local_src_cap_rc_18w.pdos,
            pd_port->local_src_cap_rc_18w.nr);
        if (ret < 0)
            pr_err("%s get source 18w pdo data fail\n", __func__);

        pr_info("%s src 18w pdo data =\n", __func__);
        for (i = 0; i < pd_port->local_src_cap_rc_18w.nr; i++) {
            pr_info("%s %d: 0x%08x\n", __func__, i,
                pd_port->local_src_cap_rc_18w.pdos[i]);
        }
...
int pd_send_hard_reset(struct pd_port *pd_port)
{
    ...
    if (NU6601_PID == chip_id) {
        tcpci_transmit(tcpc, TCPC_TX_HARD_RESET, 0, NULL);
        udelay(350);
        tcpci_pd_enable(tcpc,false);
        tcpci_pd_enable(tcpc,true);
