...
static ssize_t dump_health_desc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = ufs_read_desc_param(hba, QUERY_DESC_IDN_HEALTH, 0, HEALTH_DESC_PARAM_LEN, &buff_len, BYTE);
	if(ret) {
		count += snprintf((buf + count), PAGE_SIZE, "get device_desc fail.Fail in ufs_read_desc_param.\n");
		return count;
	}
	...
	if(ret){
		count += snprintf((buf + count), PAGE_SIZE, "get device_desc fail.Fail in ufshcd_read_desc.\n");
...
int scsi_hosin_hr(struct scsi_device *sdev, char *hr, int len)
{
	...
	buff1[3] = 0x00;
	buff1[4] = 0x43;
	result = scsi_exec_req(sdev, write_buffer, DMA_TO_DEVICE, buff1,
	...
	result = scsi_exec_req(sdev, read_buffer, DMA_FROM_DEVICE, buff2,
				  len_buff2, &sshdrr, 30 * HZ, 3, NULL);
	if (result) {
		pr_err("ufs: hr read buffer2 error 0x%x\n", result);
...
/*
	get xbstor health report
*/
int scsi_xbstor_hr(struct scsi_device *sdev, char *hr, int len){
	int ret = 0;
	struct scsi_sense_hdr /* sshdr ,*/ sshdrr;
	unsigned char read_buffer[16] = {0x3C, 0xC1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x02};
	char VU[0x2c] = {0};
	VU[0] = 0x40;
	VU[1] = 0xBB;
	if (!hr)
		return -EINVAL;
	ret = scsi_exec_req(sdev, read_buffer, DMA_FROM_DEVICE, hr,
				  len, &sshdrr, 30 * HZ, 3, NULL);
	if (ret) {
		pr_err("ufs: hr read buffer  error 0x%x\n", ret);
		pr_err("sense hr read key:0x%x; asc:0x%x; ascq:0x%x\n",
			(int)sshdrr.sense_key, (int)sshdrr.asc, (int)sshdrr.ascq);
		return -EIO;
	}
	return ret;
}
...
{
	int result;
	struct scsi_sense_hdr sshdrr;
	int len_buff = 512;
	char *buff = NULL;

	unsigned char read_buffer[16] = {0x3c, 0xc1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x02};
	if (!hr) {
		pr_err("ufs: hr is null !\n");
		return -EINVAL;
	}

	buff = kzalloc(len_buff, GFP_KERNEL);
	if (!buff) {
		pr_err("%s: ymtc health report kzalloc buff failed !\n", __func__);
		return -ENOMEM;
	}
	result = scsi_exec_req(sdev, read_buffer, DMA_FROM_DEVICE, buff,
...
static int ufs_get_xbstor_hr(struct scsi_device *sdev,  char *buf, int len){
	int ret = 0;
	unsigned long flags = 0;
	struct ufs_hba *hba = shost_priv(sdev->host);
	spin_lock_irqsave(hba->host->host_lock, flags);
	...
	if (ret)
		return ret;
	hba->host->eh_noresume = 1;
	ret = scsi_xbstor_hr(sdev, buf, len);
	scsi_device_put(sdev);
	hba->host->eh_noresume = 0;
	return ret;
}
static ssize_t hr_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	} else if (!strncmp(sdev->vendor, "XBSTOR", 6)) {
		err = ufs_get_xbstor_hr(sdev, hr, len);
	} else {
